...
var TestObjectItem = require('./TestObjectItem.react.js');
var TestObjectCreator = require('./TestObjectCreator.react.js');
...
export default class TestObjectList extends ParseComponent {
  observe(props, state) {
    return {
      items: new Parse.Query('TestObject').ascending('createdAt')
    };
  }
  ...
  render() {
    ...
    return (
      <div className={this.pendingQueries().length ? 'test_object_list loading' : 'test_object_list'}>
        ...
        {this.data.items.map(function(i) {
          ...
          // with TestObjectItem components.
          return (
            <TestObjectItem key={i.id} item={i} update={this._updateItem} destroy={this._destroyItem} />
          );
        }, this)}
        <TestObjectCreator submit={this._createItem} />
      </div>
    );
  }
  ...
  _createItem(text) {
    ParseReact.Mutation.Create('TestObjectItem', {
      text: text
    }).dispatch();
  }
